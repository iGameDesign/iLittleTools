                    ...
                    if len(lines[index]) > 0:
                        ...
                        if results2 and len(results2) > 0:
                            ...
                            if results2[0] == '1':
                                ...
                                bRet = False
                                szMsg = szMsg + "\t[Rule 1] Failed: the value of 'textureFormat' with buildTarget 'Standalone' not match. Expected(1) Actual(%s).\n" % (results2[0])
                                print lines[index]
                                lines[index] = re.sub(regx2, "filterMode: 1", lines[index])
                                print lines[index]
                        else:
                            bRet = False
                            szMsg = szMsg + "\t[Rule 1] Failed: 'textureFormat' not exists or have no vaule with buildTarget 'Standalone'.\n"
                        pass
            results = re.findall(regx3, lines[index])
            if results and len(results) > 0:
                if results[0] == 'iPhone':
                    # print results[0]
                    index = index + 2
                    if len(lines[index]) > 0:
                        results2 = re.findall(regx4, lines[index])
                        if results2 and len(results2) > 0:
                            # print results2[0]
                            if results2[0] == '33':
                                # bRet = True
                                ...
                                bRet = False
                                szMsg = szMsg + "\t[Rule 2] Failed: the value of 'textureFormat' with buildTarget 'iPhone' not match. Expected(33) Actual(%s).\n" % (results2[0])
                                print lines[index]
                                lines[index] = re.sub(regx4, "textureFormat: 33", lines[index])
                                print lines[index]
                        else:
                            bRet = False
                    ...
                    if len(lines[index]) > 0:
                        ...
                        if results2 and len(results2) > 0:
                            ...
                            if results2[0] == '12':
                                ...
                                bRet = False
                                szMsg = szMsg + "\t[Rule 3] Failed: the value of 'textureFormat' with buildTarget 'Android' not match. Expected(12) Actual(%s).\n" % (results2[0])
                                print lines[index]
                                lines[index] = re.sub(regx4, "textureFormat: 12", lines[index])
                                print lines[index]
                        else:
                            bRet = False
